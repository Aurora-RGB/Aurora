...
public static class EnumUtils {
    ...
    private static readonly Dictionary<Type, List<Func<Enum, string?>>> CustomDescriptionResolvers = new();
    ...
    static EnumUtils() {
        ...
        RegisterCustomDescriptionResolver<DeviceKeys>(e =>
        {
            var deviceKeys = (DeviceKeys)e;
            return DeviceKeysExtensions.GetDescription(deviceKeys);
        });
    }
    ...
    public static IEnumerable<KeyValuePair<string, T>> GetEnumItemsSource<T>() where T : Enum =>
    ...
    /// <summary>Takes a particular type of enum and returns all values of the enum and their associated description in a list suitable for use as an ItemsSource.
    /// Returns enumerable of KeyValuePairs where the key is the description/name and the value is the enum's value.</summary>
    /// <param name="enumType">The type of enum whose values to fetch.</param>
    public static IEnumerable<KeyValuePair<string, Enum>> GetEnumItemsSource(Type enumType) =>
        enumType.GetEnumValues().Cast<Enum>().Select(@enum => new KeyValuePair<string, Enum>(
    ...
    /// </summary>
    public static T TryParseOr<T>(string value, bool ignoreCase = true, T defaultValue = default) where T : struct, Enum, IConvertible {
        return Enum.TryParse<T>(value, ignoreCase, out var res) ? res : defaultValue;
    }

    /// <summary>
    /// Registers a new custom enum description resolver. If <see cref="GetDescription(Enum)"/> is called on a enum of the given enum type, the
    ...
    public static void RegisterCustomDescriptionResolver<TEnum>(Func<Enum, string?> resolver) where TEnum : Enum {
        ...
        if (!CustomDescriptionResolvers.ContainsKey(enumType))
            CustomDescriptionResolvers.Add(enumType, []);
    ...
    private static string? RunCustomDescriptionResolvers(Enum enumValue) {
        if (!CustomDescriptionResolvers.TryGetValue(enumValue.GetType(), out var resolverList)) return null;
...
/// </summary>
public class EnumToItemsSourceExtension(Type enumType) : MarkupExtension
{
    public bool DoGroup { get; set; } = false;

    public override object ProvideValue(IServiceProvider serviceProvider) => GetListFor(enumType, DoGroup);
