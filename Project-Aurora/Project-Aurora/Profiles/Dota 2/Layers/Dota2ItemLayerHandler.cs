...
public partial class Dota2ItemLayerHandlerProperties : LayerHandlerProperties2Color<Dota2ItemLayerHandlerProperties>
{
    ...
    [JsonProperty("_EmptyItemColor")]
    public Color EmptyItemColor
    {
        get => Logic?._EmptyItemColor ?? _emptyItemColor ?? Color.Empty;
        set => _emptyItemColor = value;
    }

    private Color? _itemsColor;

    [JsonProperty("_ItemsColor")]
    public Color ItemsColor
    {
        get => Logic?._ItemsColor ?? _itemsColor ?? Color.Empty;
        set => _itemsColor = value;
    }

    private bool? _useItemColors;

    [JsonProperty("_UseItemColors")]
    public bool UseItemColors
    {
        get => Logic?._UseItemColors ?? _useItemColors ?? false;
        set => _useItemColors = value;
    }

    private Color? _itemCooldownColor;

    [JsonProperty("_ItemCooldownColor")]
    public Color ItemCooldownColor
    {
        get => Logic?._ItemCooldownColor ?? _itemCooldownColor ?? Color.Empty;
        set => _itemCooldownColor = value;
    }

    private Color? _itemNoChargersColor;

    [JsonProperty("_ItemNoChargersColor")]
    public Color ItemNoChargersColor
    {
        get => Logic?._ItemNoChargersColor ?? _itemNoChargersColor ?? Color.Empty;
        set => _itemNoChargersColor = value;
    }

    private List<DeviceKeys>? _itemKeys;
...
public class Dota2ItemLayerHandler() : LayerHandler<Dota2ItemLayerHandlerProperties>("Dota 2 - Items")
{
    ...
    public override EffectLayer Render(IGameState state)
    {
        ...
        if (dota2State.Map.GameState is DOTA_GameState.DOTA_GAMERULES_STATE_PRE_GAME or DOTA_GameState.DOTA_GAMERULES_STATE_GAME_IN_PROGRESS)
        {
            for (var index = 0; index < dota2State.Items.InventoryCount; index++)
            {
                var item = dota2State.Items.GetInventoryAt(index);
                var key = Properties.ItemKeys[index];
            ...
            for (var index = 0; index < dota2State.Items.StashCount; index++)
            {
                var item = dota2State.Items.GetStashAt(index);
                var key = Properties.ItemKeys[9 + index];
