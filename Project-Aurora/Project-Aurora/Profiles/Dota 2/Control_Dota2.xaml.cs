...
public partial class Control_Dota2
{
    ...
    private void preview_respawn_timer_Tick(object? sender, EventArgs e)
    {
        Dispatcher.Invoke(
            () =>
            {
                if (_respawnTime < 0)
                {
                    (_profileManager.Config.Event._game_state as GameState_Dota2).Hero.IsAlive = true;
                ...
                else
                {
                    ...
                    (_profileManager.Config.Event._game_state as GameState_Dota2).Hero.SecondsToRespawn = _respawnTime;
    ...
    {
        (_profileManager.Config.Event._game_state as GameState_Dota2).Player.Team = (PlayerTeam)preview_team.SelectedItem;
    }
    ...
    private void preview_health_slider_ValueChanged(object? sender, RoutedPropertyChangedEventArgs<double> e)
    {
        ...
        (_profileManager.Config.Event._game_state as GameState_Dota2).Hero.Health = hp_val;
        (_profileManager.Config.Event._game_state as GameState_Dota2).Hero.MaxHealth = 100;
        (_profileManager.Config.Event._game_state as GameState_Dota2).Hero.HealthPercent = hp_val;
    }
    ...
    private void preview_mana_slider_ValueChanged(object? sender, RoutedPropertyChangedEventArgs<double> e)
    {
        ...
        (_profileManager.Config.Event._game_state as GameState_Dota2).Hero.Mana = mana_val;
        (_profileManager.Config.Event._game_state as GameState_Dota2).Hero.MaxMana = 100;
        (_profileManager.Config.Event._game_state as GameState_Dota2).Hero.ManaPercent = mana_val;
    }

    private void preview_killplayer_Click(object? sender, RoutedEventArgs e)
    {
        (_profileManager.Config.Event._game_state as GameState_Dota2).Hero.IsAlive = false;

        _respawnTime = 15;
        (_profileManager.Config.Event._game_state as GameState_Dota2).Hero.SecondsToRespawn = _respawnTime;
        preview_killplayer.IsEnabled = false;
        (_profileManager.Config.Event._game_state as GameState_Dota2).Player.KillStreak = _killstreak = 0;
    ...
    {
        (_profileManager.Config.Event._game_state as GameState_Dota2).Player.KillStreak = _killstreak++;
        (_profileManager.Config.Event._game_state as GameState_Dota2).Player.Kills++;
        preview_killstreak_label.Content = "Killstreak: " + _killstreak;
    }
