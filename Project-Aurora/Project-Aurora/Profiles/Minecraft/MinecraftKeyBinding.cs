...
public class MinecraftKeyBinding
{
    ...
    /// </summary>
    public DeviceKeys[] AffectedKeys { get; }

    [JsonConstructor]
    public MinecraftKeyBinding(int keyCode, string? modifier, string? context)
    {
        KeyCode = keyCode;
        Modifier = modifier;
        Context = context;
        var deviceKey = JavaKeyboardCodeToDeviceKey(keyCode);

        HashSet<DeviceKeys> keys = [deviceKey];
        switch (Modifier)
        {
            case "SHIFT":
                keys.Add(DeviceKeys.LEFT_SHIFT);
                keys.Add(DeviceKeys.RIGHT_SHIFT);
                break;
            case "CONTROL":
                keys.Add(DeviceKeys.LEFT_CONTROL);
                keys.Add(DeviceKeys.RIGHT_CONTROL);
                break;
            case "ALT":
                keys.Add(DeviceKeys.LEFT_ALT);
                keys.Add(DeviceKeys.RIGHT_ALT);
                break;
        }

        AffectedKeys = keys.ToArray();
    }
    ...
    {
        if (!ContextConflicts(other))
            return false;
        if (ModifierMatchesKeyCode(other))
            return true;
        if (KeyCode == other.KeyCode)
            return Modifier == "NONE" || other.Modifier == "NONE";
        return false;
    }
    ...
    public bool ModifierConflictsWith(MinecraftKeyBinding other)
    {
        ...
        return ContextConflicts(other) && ModifierMatchesKeyCode(other);
    }

    /// <summary>
    /// Returns true if the context for this keybinding conflicts with another context.
    /// </summary>
    internal bool ContextConflicts(MinecraftKeyBinding other)
    {
    ...
    /// </summary>
    internal bool ModifierMatchesKeyCode(MinecraftKeyBinding other)
    {
        var keyCode = other.KeyCode;
        return Modifier switch
        {
            "CONTROL" => keyCode is 0x1D or 0x9D, // 0x1D = LControl, 0x9D = RControl
            "SHIFT" => keyCode is 0x2A or 0x36, // 0x2A = LShift, 0x36 = RShift
            "ALT" => keyCode is 0x38 or 0xB8, // 0x38 = LAlt, 0xB8 = RAlt
            _ => false
        };
    }
