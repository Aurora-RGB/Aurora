...
/// </summary>
public partial class Control_EvaluatablePresenter
{
    ...
    public static readonly DependencyProperty ExpressionProperty =
        DependencyProperty.Register(nameof(Expression), typeof(IEvaluatable), typeof(Control_EvaluatablePresenter), new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.AffectsRender, OnExpressionChange));
    ...
    private static void OnExpressionChange(DependencyObject evaluatablePresenter, DependencyPropertyChangedEventArgs eventArgs) {
        ...
        control.OnExpressionChanged(eventArgs.OldValue, eventArgs.NewValue);
        ...
        if (expr?.LastQuery == 0)
        {
            control.ChangeIndicator.Opacity = 0;
        }
    }
    
    private void OnExpressionChanged(object? oldValue, object? newValue)
    {
        if (oldValue is INotifyPropertyChanged oldNpc)
            oldNpc.PropertyChanged -= Expression_PropertyChanged;

        if (newValue is INotifyPropertyChanged newNpc)
            newNpc.PropertyChanged += Expression_PropertyChanged;
    }

    private void Expression_PropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        if (e.PropertyName != nameof(IEvaluatable.LastQuery))
        {
            return;
        }
    ...
    public static readonly DependencyProperty EvalTypeProperty =
        DependencyProperty.Register(nameof(EvalType), typeof(Type), typeof(Control_EvaluatablePresenter),
            new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.AffectsRender, null, CoerceEvalType));
    ...
    {
        if (!TryGetData(e.Data, out var eval, out var source)) return;
        ...
        if (source != null && ctrlKey) {
            ...
            // We also need to call the changed event on the other presenter
            // (since this is not done by the DependencyProperty to prevent a invoker-subscriber loop with WPF updating itself)
    ...
    private long _lastAnim;
    private void AnimateChangeIndicator()
    {
        if (_lastAnim > Environment.TickCount - 150)
        {
            return;
        }
        Dispatcher.Invoke(() =>
        {
            // Reset opacity instantly (important when animations fire rapidly)
            //ChangeIndicator.BeginAnimation(OpacityProperty, null, HandoffBehavior.SnapshotAndReplace);
            //ChangeIndicator.Opacity = 1;
            DebugValText.Text = Expression?.LastValue?.ToString() ?? "null";
            ...
            ChangeIndicator.BeginAnimation(OpacityProperty, fadeOut, HandoffBehavior.SnapshotAndReplace);
            _lastAnim = Environment.TickCount;
    ...
    private bool TryGetData(IDataObject @do, [MaybeNullWhen(false)] out IEvaluatable evaluatable, out Control_EvaluatablePresenter? source)
        => EvaluatableHelpers.TryGetData(@do, out evaluatable, out source, EvalType);
    #endregion

    private void Control_EvaluatablePresenter_OnUnloaded(object sender, RoutedEventArgs e)
    {
        // unsubscribe from the expression's property changed event to prevent memory leaks
        if (Expression is INotifyPropertyChanged npc)
            npc.PropertyChanged -= Expression_PropertyChanged;
    }
}
...
public static class EvaluatablePresenterAddons {
    private const FrameworkPropertyMetadataOptions A = FrameworkPropertyMetadataOptions.AffectsRender | FrameworkPropertyMetadataOptions.AffectsArrange |
    ...
    public static readonly DependencyProperty ShowDebugInfoProperty =
        DependencyProperty.RegisterAttached("ShowDebugInfo", typeof(bool), typeof(EvaluatablePresenterAddons),
            new FrameworkPropertyMetadata(false, A));
}